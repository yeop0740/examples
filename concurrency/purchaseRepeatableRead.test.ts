...
import { purchase as purchaseRepeatableRead } from "./purchaseRepeatableRead";

describe("purchaseRepeatableRead 는", () => {
  ...
  it("한 번의 구매에 대해서 처음 금액에서 사용한 만큼 감소시킨 양이 account detail 에 저장된다.", async () => {
    ...
    // when
    await purchaseRepeatableRead(createdAccount.id, changeAmount, prisma);
  ...
  it("잔액 부족 시 에러를 발생시킨다.", async () => {
    ...
    // then
    await expect(
      purchaseRepeatableRead(createdAccount.id, changeAmount, prisma)
  ...
  it("잔액 1 소진에 대한 구매 100회 실행 시 100 만큼 잔액이 소진된다.", async () => {
    ...
    for (let i = 0; i < numberOfTrials; i++) {
      const promise = purchaseRepeatableRead(
        createdAccount.id,
        changeAmount,
        prisma
      );
